...
def _load_append_jsonl():
    ...
    spec = importlib.util.spec_from_file_location('claude_hooks_utils_bridge_audit', module_path)
...
def handle_hook(context) -> None:
    outputs_root = Path.cwd() / '.claude' / 'hooks' / 'outputs' / 'bridge'
    append_jsonl(outputs_root / 'bridge_events.jsonl', context.to_dict())

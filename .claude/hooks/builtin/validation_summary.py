...
def _load_append_jsonl():
    ...
    spec = importlib.util.spec_from_file_location('claude_hooks_utils_validation_summary', module_path)
...
def handle_hook(context) -> None:
    outputs_root = Path.cwd() / '.claude' / 'hooks' / 'outputs' / 'validation'
    append_jsonl(outputs_root / 'validation_events.jsonl', context.to_dict())
